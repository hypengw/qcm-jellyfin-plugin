...
namespace model
{
using return_type_getMusicGenre = OAIBaseItemDto;
using return_type_getMusicGenres = OAIBaseItemDtoQueryResult;
}

namespace api::param 
{
struct getMusicGenre {
    std::string genreName; // The genre name.
    std::optional<std::string> userId; // Optional. Filter by user id, and attach user data.
};
struct getMusicGenres {
...
struct getMusicGenre {
public:
    using in_type                      = param::getMusicGenre;
    using out_type                     = model::return_type_getMusicGenre;
    constexpr static Operation  oper   = Operation::GET;
    auto path() const { 
        return fmt::format("/MusicGenres/{}"sv,
            input.genreName
        );
    }
...
struct getMusicGenres {
public:
    using in_type                      = param::getMusicGenres;
    using out_type                     = model::return_type_getMusicGenres;
    constexpr static Operation  oper   = Operation::GET;
    auto path() const { 
        return "/MusicGenres"sv; 
    }
